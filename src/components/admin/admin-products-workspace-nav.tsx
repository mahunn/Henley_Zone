...
export function AdminProductsWorkspaceNav() {
  ...
  const value = pathname.includes("/admin/categories")
    ? "/admin/categories"
    : pathname.includes("/admin/products/manage")
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      <label style={{ display: "flex", flexDirection: "column", gap: 6, maxWidth: 400 }}>
        <span style={{ fontWeight: 600 }}>Admin workspace</span>
        <select
        ...
        >
          ...
          <option value="/admin/categories">Manage categories & nav order</option>
        </select>
      </label>
    </div>
  );
